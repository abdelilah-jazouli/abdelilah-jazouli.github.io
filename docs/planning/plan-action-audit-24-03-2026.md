...
| 1.3 | Supprimer le code JS de soumission simulee (`setTimeout` + `fetch` commente) dans `initContactForm()` | `js/main.js` | `[ ]` |
| 1.4 | Supprimer le code Calendly residuel commente dans la section contact | `index.html` | `[ ]` |
...
|---|-------|------------|--------|--------|
| 1.5 | `🎨` Prototyper le design des pages legales via Stitch (fond sombre, glass card contenu, typographie coherente) | `.stitch/designs/` | `ui-design` | `[ ]` |
| 1.6 | `🔁` Generer `mentions-legales.html` via stitch-loop (raison sociale, SIRET, hebergeur, directeur de publication) | `mentions-legales.html` | `loop` | `[ ]` |
| 1.7 | `🔁` Generer `politique-confidentialite.html` via stitch-loop (finalites, base legale, duree de conservation, droits RGPD) | `politique-confidentialite.html` | `loop` | `[ ]` |
| 1.8 | Mettre a jour les liens du footer sur les 3 pages (`index.html`, `blog.html`, `article.html`) pour pointer vers les nouvelles pages | `index.html`, `blog.html`, `article.html` | — | `[ ]` |
| 1.9 | Mettre a jour le copyright footer de 2025 a 2026 sur les 3 pages | `index.html`, `blog.html`, `article.html` | — | `[ ]` |

### 2.3 Liens morts et incoherences

| # | Tache | Fichier(s) | Stitch | Statut |
|---|-------|------------|--------|--------|
| 1.10 | Corriger les liens LinkedIn et GitHub dans le footer (pointer vers les vrais profils comme dans la section contact) sur les 3 pages | `index.html`, `blog.html`, `article.html` | — | `[ ]` |
| 1.11 | Supprimer le `target="_blank"` sur le lien blog interne dans la section contact | `index.html` | — | `[ ]` |
...
| # | Tache | Fichier(s) | Statut |
|---|-------|------------|--------|
| 2.1 | Creer une image OG officielle (1200x630px) a partir de la banniere hero | `images/og-image.png` | `[ ]` |
| 2.2 | Ajouter les balises `og:image`, `og:image:width`, `og:image:height` sur `index.html` | `index.html` | `[ ]` |
| 2.3 | Ajouter les balises Twitter Card (`twitter:card`, `twitter:title`, `twitter:description`, `twitter:image`) sur `index.html` | `index.html` | `[ ]` |
| 2.4 | Ajouter les meta OG et Twitter dynamiques dans `blog.html` et `article.html` (via JS pour article) | `blog.html`, `article.html`, `js/blog.js` | `[ ]` |

### 3.2 Indexation et crawl

| # | Tache | Fichier(s) | Statut |
|---|-------|------------|--------|
| 2.5 | Creer `robots.txt` (Allow all, lien vers sitemap) | `robots.txt` | `[ ]` |
| 2.6 | Creer `sitemap.xml` (pages statiques + articles blog) | `sitemap.xml` | `[ ]` |
| 2.7 | Ajouter `<link rel="canonical">` sur chaque page | `index.html`, `blog.html`, `article.html` | `[ ]` |

### 3.3 Donnees structurees

| # | Tache | Fichier(s) | Statut |
|---|-------|------------|--------|
| 2.8 | Ajouter un bloc JSON-LD `Organization` dans `index.html` (nom, logo, url, sameAs) | `index.html` | `[ ]` |

### 3.4 Contenu blog

| # | Tache | Fichier(s) | Statut |
|---|-------|------------|--------|
| 2.9 | Renseigner le champ `description` dans le front-matter YAML de chaque article, puis regenerer `posts/index.json` | `posts/*.md`, `posts/index.json` | `[ ]` |
